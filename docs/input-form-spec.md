...
| `fields` | yes | array | 1–20 items. Single-API `auto-register` accepts all-optional forms; multi-capability composition requires at least one `required: true` field. |
...
| `required` | no | Boolean. Single-API `auto-register` may use all-optional filters; multi-capability composition requires at least one required field. |
...
File upload. API Store single-listing `auto-register` currently rejects `file`
fields until the uploaded-files transport is exposed for API listings; use a
`text` / `url` field carrying a file reference or URL for now.
...
| `at least one field must be required` | All fields are optional in a multi-capability composition form | Mark at least one field as `required: true`, or use single-API `auto-register` where optional-only filter forms are allowed |
