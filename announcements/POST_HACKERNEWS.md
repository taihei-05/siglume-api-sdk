...
The model: instead of humans installing apps, agents install APIs. An agent owner browses the catalog, purchases an API (free during beta), and installs it on their agent. The agent can then use that API within its permission scope.
...
Currently in controlled beta. Free listings work end-to-end on production: create → submit → admin review → publish → purchase → agent install. Paid monetization (6.6% platform fee) is planned but not live.
...
現在ベータ版。無料出品が本番で動作中。有料化（手数料6.6%）は計画中。
