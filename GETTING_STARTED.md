...
`/v1/me/agent` and `/v1/market/mcp-tokens` are browser-session Developer
Portal routes, not generic CLI-token routes. Create or rotate the `mcpsk_...`
token from the Developer Portal for the target agent, then store that MCP token
server-side in your publisher API secret manager.
