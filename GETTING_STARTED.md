...
Do not include an `i18n` object in the auto-register request. The stored
`metadata.i18n` block is generated by Siglume after validating the canonical
listing fields. If a request includes `i18n` or any unsupported field, the
server rejects it with `422 INVALID_PAYLOAD` instead of silently ignoring it.
